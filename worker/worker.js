/**
 * FlimmerKanal.de — Kostenloser Test Worker
...
// RESEND_KEY is stored as a Cloudflare Worker Secret (env.RESEND_KEY)
  ...
  <tr><td align="center">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="background-color:${DARK};padding:32px 40px;text-align:center;border-bottom:3px solid ${RED};">
        <h1 style="margin:0;font-family:Arial,sans-serif;font-size:26px;font-weight:bold;color:#ffffff;">FlimmerKanal</h1>
      ...
      <tr><td style="background-color:#f8f8f8;border-top:1px solid #eeeeee;padding:18px 40px;text-align:center;">
        <p style="margin:0;font-family:Arial,sans-serif;font-size:11px;color:#aaaaaa;">
          © 2026 FlimmerKanal · <a href="${SITE_URL}" style="color:${RED};text-decoration:none;">flimmerkanal.de</a>
        </p>
      </td></tr>
    </table>
  </td></tr>
...
function welcomeEmail(name, username, password, m3uUrl) {
  ...
  return emailWrap(`
    ...
    <p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#555555;">Mit freundlichen Grüßen,<br><strong>Das FlimmerKanal Team</strong></p>
  `);
}
...
function reminderEmail(name, username, password, m3uUrl) {
  ...
  return emailWrap(`
    ...
    ${ctaButton("Jetzt Abonnement wählen →", SITE_URL + "/preise.html")}
    ...
    <p style="margin:0 0 20px;font-family:Arial,sans-serif;font-size:14px;line-height:1.65;color:#555555;">
      ...
      <a href="https://wa.me/${WA_NUMBER}" style="color:${RED};text-decoration:none;font-weight:bold;">+1 782-802-6280</a> — wir sind immer für Sie da.
    </p>
    <p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#555555;">Mit freundlichen Grüßen,<br><strong>Das FlimmerKanal Team</strong></p>
  `);
}
...
function followupEmail(name) {
  ...
  return emailWrap(`
    ...
    ${ctaButton("Mein Abonnement wählen →", SITE_URL + "/preise.html")}
    <p style="margin:0 0 20px;font-family:Arial,sans-serif;font-size:14px;line-height:1.65;color:#555555;">
      Eine Frage? Antworten Sie auf diese E-Mail oder schreiben Sie uns auf WhatsApp:
      <a href="https://wa.me/${WA_NUMBER}" style="color:${RED};text-decoration:none;font-weight:bold;">+1 782-802-6280</a> — wir würden uns freuen, Sie zu behalten.
    </p>
    <p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#555555;">Mit freundlichen Grüßen,<br><strong>Das FlimmerKanal Team</strong></p>
  `);
}
...
<body style="font-family:Arial,sans-serif;font-size:14px;color:#333;padding:20px;">
  <h2 style="color:${RED};margin-top:0;">Neuer Testzugang — FlimmerKanal</h2>
...
async function handleFetch(request, env) {
  ...
  if (request.method === "GET") {
    ...
    return new Response("FlimmerKanal Trial Worker — OK", { status: 200 });
  }
  ...
  try {
    ...
    const crRes = await apiGet({
      ...
      note: `Trial / flimmerkanal.de / ${email} | ${whatsapp || ""}`,
    });
    ...
    step = "email_client";
    await sendEmail(email, "Ihr FlimmerKanal Testzugang ist bereit – 24h Gratis aktiviert ✓", welcomeEmail(name, username, password, m3uUrl), env.RESEND_KEY);

    step = "email_admin";
    await sendEmail(ADMIN_EMAIL, `Automation / flimmerkanal.de / trial / ${name || "—"} / ${email}`, adminEmail(name, email, country, device, whatsapp, notes, username, password, m3uUrl), env.RESEND_KEY);
...
async function handleScheduled(env) {
  ...
  for (const { name: key } of keys) {
    ...
    if (!reminder_sent && now >= expiry - FOUR_HOURS && now < expiry) {
      try {
        await sendEmail(email, "⏳ Ihr FlimmerKanal Testzugang läuft in 4 Stunden ab", reminderEmail(name, username, password, m3uUrl), env.RESEND_KEY);
    ...
    if (!followup_sent && now >= expiry) {
      try {
        await sendEmail(email, "Ihr FlimmerKanal Testzugang ist abgelaufen — Jetzt weiterschauen 🎬", followupEmail(name), env.RESEND_KEY);
